...
from .model import LSTMBPTTConfig, LSTMRegressor
...
class LSTMBenchmarkConfig:
    ...
    out_dir: str = os.path.join(tempfile.gettempdir(), "lstm_bptt_plots")
    ...
    *,
    ...
    cfg = LSTMBenchmarkConfig(
        ...
        out_dir=(
            ...
            if out_dir
            else _ensure_out_dir(LSTMBenchmarkConfig().out_dir)
        ),
    ...
    _log(f"[LSTM] Device: {cfg.device}, Seed: {cfg.seed}", verbose=cfg.verbose)
    _log(f"[LSTM] Tasks: {sorted(list(task_set))}", verbose=cfg.verbose)
    _log(
        f"[LSTM] Regime: narma_mode={cfg.narma_mode}, multi-step H_max={cfg.ms_horizon_max}",
        verbose=cfg.verbose,
    )
    _log(
        f"[LSTM] Plots dir: {cfg.out_dir} | produce_plots={cfg.produce_plots}",
        verbose=cfg.verbose,
    )
