...
from .run import run_lstm_benchmark
...
def run_search(task: str, max_evals: int = 30, seed: int = 42, verbose: bool = True):
    ...
    _log(f"[LSTM-TPE] task={task} max_evals={max_evals} device={device}", verbose)
    ...
    yaml_path = comp_dir / f"{task}_lstm.yaml"
    plot_dir = comp_dir / f"{task}_lstm_plots"
    ...
    _log(f"[LSTM-TPE] Saved: {yaml_path}", verbose)
    _log("[LSTM-TPE] Generating plots...", verbose)
